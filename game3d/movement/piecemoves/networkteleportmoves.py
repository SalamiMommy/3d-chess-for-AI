...
def generate_network_teleport_with_king_moves(state: 'GameState', x: int, y: int, z: int) -> List[Move]:
    ...
    """
    teleport_moves = generate_network_teleport_moves(state.cache, state.color, x, y, z)
...
def network_teleport_move_dispatcher(state: 'GameState', x: int, y: int, z: int) -> List[Move]:
