...
def generate_mirror_teleport_with_king_moves(state: GameState, x: int, y: int, z: int) -> List[Move]:
    """
    Combines mirror-teleport move + 1-step king moves.
...
@register(PieceType.MIRROR_TELEPORT)
def mirror_teleport_dispatcher(state: GameState, x: int, y: int, z: int) -> List[Move]:
    """
