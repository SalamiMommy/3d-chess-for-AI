...
# ------------------------------------------------------------------
# Debugging and validation utilities - NUMPY OPTIMIZED VERSION
# ------------------------------------------------------------------
...
def validate_coord(c: Coord) -> bool:
    return (isinstance(c, tuple) and len(c) == 3 and
            all(isinstance(i, int) for i in c) and
            in_bounds_vectorised(np.array([c]))[0])

def coord_to_string(c: Coord) -> str:
    return f"({c[0]}, {c[1]}, {c[2]})"

def batch_coords_to_array(coords: List[Coord]) -> np.ndarray:
    return np.array(coords, dtype=np.int8) if coords else np.empty((0, 3), dtype=np.int8)
