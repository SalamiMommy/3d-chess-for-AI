...
from game3d.cache.effectscache.blackholesuckcache import (
    BlackHoleSuckCache, init_black_hole_suck_cache, get_black_hole_suck_cache
)
from game3d.cache.effectscache.movementdebuffcache import (
    MovementDebuffCache, init_movement_debuff_cache, get_movement_debuff_cache
)
from game3d.cache.effectscache.movementbuffcache import (
    MovementBuffCache, init_movement_buff_cache, get_movement_buff_cache
)
from game3d.cache.effectscache.whiteholepushcache import (
    WhiteHolePushCache, init_white_hole_push_cache, get_white_hole_push_cache
)
from game3d.cache.effectscache.trailblazecache import (
    TrailblazeCache, init_trailblaze_cache, get_trailblaze_cache
)
from game3d.cache.effectscache.capturefrombehindcache import (
    CaptureFromBehindCache, init_capturefrombehind_cache, get_capturefrombehind_cache
)
...
from game3d.cache.effectscache.armouredcache import ArmouredCache, init_armoured_cache, get_armoured_cache
...
class CacheManager:
    def __init__(self, board: Board) -> None:
        ...
        self._effect: Dict[str, object] = {
            ...
            "movement_buff":   MovementBuffCache(board),
            "movement_debuff": MovementDebuffCache(board),
            "black_hole_suck": BlackHoleSuckCache(board),
            "white_hole_push": WhiteHolePushCache(board),
            "trailblaze":      TrailblazeCache(board),   # <-- NEW
            "archery": ArcheryCache(board),
            "armoured": ArmouredCache(board),
            "share_square": ShareSquareCache(board),
            "capture_from_behind": CaptureFromBehindCache(board),
        }
    ...
    def current_trail_squares(self, controller: Color) -> Set[Tuple[int, int, int]]:
        """All squares blazing for controller (used by GameState to increment counters)."""
    ...
    def block_square(self, sq: Tuple[int, int, int], current_ply: int) -> bool:
        """Called by GameState when geomancer submits a block."""
    ...
    def pieces_at(self, sq: Tuple[int, int, int]) -> List[Piece]:
        """Multi-occupancy list (bottom first)."""
    ...
    def can_capture_wall_from_behind(self, attacker_sq: Tuple[int, int, int], wall_sq: Tuple[int, int, int], controller: Color) -> bool:
        return self._effect["capture_from_behind"].can_capture(attacker_sq, wall_sq, controller)
# ------------------------------------------------------------------
