...
from game3d.cache.transposition import TranspositionTable, TTEntry, CompactMove
...
class SymmetryAwareTranspositionTable(TranspositionTable):
    ...
    def __init__(self, symmetry_manager: SymmetryManager, size_mb: int = 6144):  # 🔥 6 GB default
    ...
    def probe_with_symmetry(self, hash_value: int, board) -> Optional[TTEntry]:
        ...
        if canonical_entry:
            ...
            self._canonical_cache.put(hash_value, (canonical_hash, canonical_transform.name))
            return self._transform_entry_back(canonical_entry, canonical_transform.name)
    ...
    def store_with_symmetry(self, hash_value: int, board, depth: int,
        ...
        if depth >= 3:
            try:
                ...
                if best_move and canonical_transform.name != "identity":
                    canonical_move = self._transform_move(best_move, canonical_transform.name)
    ...
    def _compute_zobrist_hash(self, board) -> int:
        ...
        from game3d.cache.movecache import ZobristHashing
        hasher = ZobristHashing()
        return hasher.compute_hash(board, Color.WHITE)  # Side doesn't matter for canonical
    ...
    def _transform_move(self, move: CompactMove, transform_name: str) -> CompactMove:
        ...
        # Reconstruct piece type (assumes piece type unchanged by rotation)
        from game3d.pieces.enums import PieceType
        piece_type = PieceType.PAWN  # Placeholder — you may need to store this in CompactMove
        captured_type = PieceType.PAWN if is_capture else None
    ...
    def _transform_move_back(self, move: CompactMove, transform_name: str) -> CompactMove:
        """Transform move FROM canonical back to original."""
        if transform_name == "identity":
            return move
        inverse_name = self._get_inverse_transform_name(transform_name)
        return self._transform_move(move, inverse_name)

    def _get_inverse_transform_name(self, name: str) -> str:
        for rot_type, inv_type in self._inverse_transform_map.items():
            if rot_type.value == name:
                return inv_type.value
        return "identity"
