# game3d/movement/pieces/bigknights.py - FIXED
"""
...
def knight31_move_dispatcher(state: 'GameState', x: int, y: int, z: int) -> List[Move]:
    ...
    return generate_knight31_moves(state.cache_manager, state.color, x, y, z)

@register(PieceType.KNIGHT32)
def knight32_move_dispatcher(state: 'GameState', x: int, y: int, z: int) -> List[Move]:
    x, y, z = ensure_int_coords(x, y, z)
    # FIXED: Use cache_manager property
    return generate_knight32_moves(state.cache_manager, state.color, x, y, z)
